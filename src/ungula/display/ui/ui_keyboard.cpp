...
static void build_layout()
{
    ...
    const char **layout = s_symbols_mode ? s_layout_symbols : s_shift_active ? s_layout_upper : s_layout_lower;
...
static void draw_key(int idx, bool pressed)
{
    ...
    ui_draw_button(key.x, key.y, key.w, key.h, label, bg_color, UI_COLOR_TEXT_PRIMARY, UI_TEXT_SIZE_NORMAL,
                   UI_RADIUS_SMALL);
}
...
static void draw_text_display()
{
    ...
    int disp_w = s_password_mode ? (KEYBOARD_WIDTH - padding * 2 - 72) : (KEYBOARD_WIDTH - padding * 2);
    int disp_h = 38;
...
static int find_key_at(int touch_x, int touch_y)
{
    for (int i = 0; i < s_key_count; i++) {
        if (ui_touch_in_rect(touch_x, touch_y, s_keys[i].x, s_keys[i].y, s_keys[i].w, s_keys[i].h)) {
            return i;
        }
    }
    return -1;
...
void keyboard_show(int pos_x, int pos_y, const char *title, const char *initial_text, int max_length,
                   keyboard_callback_t callback, void *user_data, bool password_mode)
{
    s_x = pos_x;
    s_y = pos_y;
...
void keyboard_draw()
{
    ...
    gfx.drawRoundRect(s_x - 3, s_y - 3, KEYBOARD_WIDTH + 6, KEYBOARD_HEIGHT + 6, UI_RADIUS_NORMAL + 3, 0x0000);
    gfx.drawRoundRect(s_x - 2, s_y - 2, KEYBOARD_WIDTH + 4, KEYBOARD_HEIGHT + 4, UI_RADIUS_NORMAL + 2, 0x0000);
    gfx.drawRoundRect(s_x - 1, s_y - 1, KEYBOARD_WIDTH + 2, KEYBOARD_HEIGHT + 2, UI_RADIUS_NORMAL + 1, 0x0000);

    // Background
    gfx.fillRoundRect(s_x, s_y, KEYBOARD_WIDTH, KEYBOARD_HEIGHT, UI_RADIUS_NORMAL, UI_COLOR_BG_PANEL);
    ...
    gfx.drawRoundRect(s_x + 1, s_y + 1, KEYBOARD_WIDTH - 2, KEYBOARD_HEIGHT - 2, UI_RADIUS_NORMAL - 1, UI_COLOR_ACCENT);
    gfx.drawRoundRect(s_x + 2, s_y + 2, KEYBOARD_WIDTH - 4, KEYBOARD_HEIGHT - 4, UI_RADIUS_NORMAL - 2, UI_COLOR_BORDER);
