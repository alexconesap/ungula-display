...
static void draw_button(int idx, bool pressed)
{
    ...
    ui_draw_button(btn.x, btn.y, btn.w, btn.h, btn.label, bg_color, UI_COLOR_TEXT_PRIMARY, UI_TEXT_SIZE_NORMAL,
                   UI_RADIUS_NORMAL);
}
...
static int find_button_at(int touch_x, int touch_y)
{
    for (int i = 0; i < KEYPAD_BTN_COUNT; i++) {
        if (ui_touch_in_rect(touch_x, touch_y, s_buttons[i].x, s_buttons[i].y, s_buttons[i].w, s_buttons[i].h)) {
            return i;
        }
    }
    return -1;
...
void keypad_show(int pos_x, int pos_y, const char *title, int initial_value, int min_value, int max_value,
                 keypad_callback_t callback, void *user_data, keypad_cancel_callback_t cancel_callback,
                 bool password_mode, int max_digits, bool anti_guess)
{
    s_x = pos_x;
    s_y = pos_y;
...
void keypad_draw()
{
    ...
    gfx.drawRoundRect(s_x - 3, s_y - 3, KEYPAD_WIDTH + 6, KEYPAD_HEIGHT + 6, UI_RADIUS_NORMAL + 3, 0x0000);
    gfx.drawRoundRect(s_x - 2, s_y - 2, KEYPAD_WIDTH + 4, KEYPAD_HEIGHT + 4, UI_RADIUS_NORMAL + 2, 0x0000);
    gfx.drawRoundRect(s_x - 1, s_y - 1, KEYPAD_WIDTH + 2, KEYPAD_HEIGHT + 2, UI_RADIUS_NORMAL + 1, 0x0000);
    ...
    gfx.drawRoundRect(s_x + 1, s_y + 1, KEYPAD_WIDTH - 2, KEYPAD_HEIGHT - 2, UI_RADIUS_NORMAL - 1, UI_COLOR_ACCENT);
    gfx.drawRoundRect(s_x + 2, s_y + 2, KEYPAD_WIDTH - 4, KEYPAD_HEIGHT - 4, UI_RADIUS_NORMAL - 2, UI_COLOR_BORDER);
