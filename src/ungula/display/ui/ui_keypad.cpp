...
static int find_button_at(int touch_x, int touch_y)
{
        for (int i = 0; i < KEYPAD_BTN_COUNT; i++) {
                if (ui_touch_in_rect(touch_x, touch_y, s_buttons[i].x, s_buttons[i].y,
        ...
        if (action >= 0 && action <= 9) {
                ...
                if (s_password_mode) {
                        ...
                        if (s_digit_count >= required) {
                                ...
                                int disp_h = 48;
                                gfx.drawRoundRect(disp_x, disp_y, disp_w, disp_h, UI_RADIUS_SMALL,
                                                  UI_COLOR_DANGER);
                        }
                } else {
                 ...
                 bool anti_guess, int decimal_places)
{
        ...
        if (anti_guess) {
                ...
                s_max_digits =
                    (max_digits > 0 && max_digits < (int)sizeof(s_digit_buf)) ? max_digits : 0;
        }
        s_digit_count = 0;
        s_digit_buf[0] = '\0';
        s_pressed_btn = -1;
...
void keypad_draw()
{
        ...
        gfx.drawRoundRect(s_x - 3, s_y - 3, KEYPAD_WIDTH + 6, KEYPAD_HEIGHT + 6,
                          UI_RADIUS_NORMAL + 3, 0x0000);
        gfx.drawRoundRect(s_x - 2, s_y - 2, KEYPAD_WIDTH + 4, KEYPAD_HEIGHT + 4,
                          UI_RADIUS_NORMAL + 2, 0x0000);
        gfx.drawRoundRect(s_x - 1, s_y - 1, KEYPAD_WIDTH + 2, KEYPAD_HEIGHT + 2,
                          UI_RADIUS_NORMAL + 1, 0x0000);

        // Draw keypad background
        gfx.fillRoundRect(s_x, s_y, KEYPAD_WIDTH, KEYPAD_HEIGHT, UI_RADIUS_NORMAL,
                          UI_COLOR_BG_PANEL);
