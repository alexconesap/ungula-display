...
static void draw_network_list()
{
        ...
        if (s_scanning) {
                ...
                gfx_set_font(UI_TEXT_SIZE_NORMAL);
                gfx_setTextColor(UI_COLOR_TEXT_SECONDARY);
                gfx_drawCentreString(wstr(WIFI_STR_SCANNING),
                                     WIFI_LIST_X + ((WIFI_LIST_W - 50) / 2),
...
static void draw_button_with_state(int btn_id)
{
        ...
        case BTN_CONNECT: {
                ...
                ui_draw_button(WIFI_BTN_START_X + WIFI_BTN_W + WIFI_BTN_GAP, btn_y, WIFI_BTN_W,
                               btn_h, wstr(WIFI_STR_BTN_CONNECT), color, UI_COLOR_TEXT_PRIMARY,
...
static void draw_enable_checkbox()
{
        ...
        // Checkbox border
        gfx_drawRoundRect(WIFI_CB_X, WIFI_CB_Y, WIFI_CB_SIZE, WIFI_CB_SIZE, 4,
                          UI_COLOR_TEXT_PRIMARY);

        // Fill if checked
        if (s_enabled) {
                gfx_fillRoundRect(WIFI_CB_X + 3, WIFI_CB_Y + 3, WIFI_CB_SIZE - 6, WIFI_CB_SIZE - 6,
                                  2, UI_COLOR_ACCENT);
        }
...
static int find_button_at(int touch_x, int touch_y)
{
        ...
        if (ui_touch_on_close_button(touch_x, touch_y, WIFI_CLOSE_X, WIFI_CLOSE_Y,
                                     WIFI_CLOSE_SIZE)) {
                return BTN_CANCEL;
        }
...
bool wifi_selector_handle_touch(int touch_x, int touch_y, bool pressed)
{
        ...
        if (!ui_touch_in_rect(touch_x, touch_y, s_x, s_y, WIFI_SELECTOR_WIDTH,
                              WIFI_SELECTOR_HEIGHT)) {
                return false;
        }
