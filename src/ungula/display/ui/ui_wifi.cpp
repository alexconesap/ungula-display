...
{
    // List background
    gfx_fillRoundRect(WIFI_LIST_X, WIFI_LIST_Y, WIFI_LIST_W - 50, WIFI_LIST_H, UI_RADIUS_NORMAL, UI_COLOR_BG_PANEL);
    gfx_drawRoundRect(WIFI_LIST_X, WIFI_LIST_Y, WIFI_LIST_W - 50, WIFI_LIST_H, UI_RADIUS_NORMAL, UI_COLOR_BORDER_DIM);
    ...
    if (s_scanning) {
        ...
        gfx_set_font(UI_TEXT_SIZE_NORMAL);
        gfx_setTextColor(UI_COLOR_TEXT_SECONDARY);
        gfx_drawCentreString(wstr(msg), WIFI_LIST_X + ((WIFI_LIST_W - 50) / 2), WIFI_LIST_Y + (WIFI_LIST_H / 2) - 8);
    } else {
        // Draw visible items
        for (int i = s_scroll_offset; i < s_network_count && i < s_scroll_offset + WIFI_VISIBLE_ITEMS; i++) {
    ...
    ui_draw_button(WIFI_SCROLL_BTN_X, WIFI_LIST_Y, WIFI_SCROLL_BTN_W, WIFI_SCROLL_BTN_W, "^",
                   can_scroll_up ? UI_COLOR_BTN_PRIMARY : UI_COLOR_ACCENT_DIM, UI_COLOR_TEXT_PRIMARY,
                   UI_TEXT_SIZE_NORMAL, UI_RADIUS_SMALL);

    ui_draw_button(WIFI_SCROLL_BTN_X, WIFI_LIST_Y + WIFI_LIST_H - WIFI_SCROLL_BTN_W, WIFI_SCROLL_BTN_W,
                   WIFI_SCROLL_BTN_W, "v", can_scroll_down ? UI_COLOR_BTN_PRIMARY : UI_COLOR_ACCENT_DIM,
...
static void draw_button_with_state(int btn_id)
{
    ...
    switch (btn_id) {
    case BTN_SCAN: {
        uint16_t color = pressed ? UI_COLOR_BTN_PRESSED : (s_scanning ? UI_COLOR_ACCENT_DIM : UI_COLOR_BTN_PRIMARY);
        ui_draw_button(WIFI_BTN_START_X, btn_y, WIFI_BTN_W, btn_h, wstr(WIFI_STR_BTN_SCAN), color,
                       UI_COLOR_TEXT_PRIMARY, UI_TEXT_SIZE_NORMAL, UI_RADIUS_NORMAL);
        break;
    }
    case BTN_CONNECT: {
        bool can_connect = (s_selected_idx >= 0 && !s_scanning);
        uint16_t color = pressed ? UI_COLOR_BTN_PRESSED : (can_connect ? UI_COLOR_SUCCESS : UI_COLOR_ACCENT_DIM);
        ui_draw_button(WIFI_BTN_START_X + WIFI_BTN_W + WIFI_BTN_GAP, btn_y, WIFI_BTN_W, btn_h,
                       wstr(WIFI_STR_BTN_CONNECT), color, UI_COLOR_TEXT_PRIMARY, UI_TEXT_SIZE_NORMAL, UI_RADIUS_NORMAL);
        break;
    }
    case BTN_CANCEL: {
...
{
    // Clear checkbox area
    gfx_fillRect(WIFI_CB_X - 2, WIFI_CB_Y - 2, WIFI_CB_TOUCH_W + 4, WIFI_CB_TOUCH_H, UI_COLOR_BG_PANEL);
    ...
    // Fill if checked
    if (s_enabled) {
        gfx_fillRoundRect(WIFI_CB_X + 3, WIFI_CB_Y + 3, WIFI_CB_SIZE - 6, WIFI_CB_SIZE - 6, 2, UI_COLOR_ACCENT);
    }
...
static int find_button_at(int touch_x, int touch_y)
{
    ...
    if (ui_touch_in_rect(touch_x, touch_y, WIFI_BTN_START_X + WIFI_BTN_W + WIFI_BTN_GAP, btn_y, WIFI_BTN_W, btn_h)) {
        return BTN_CONNECT;
    }
    ...
    if (ui_touch_in_rect(touch_x, touch_y, WIFI_SCROLL_BTN_X, WIFI_LIST_Y, WIFI_SCROLL_BTN_W, WIFI_SCROLL_BTN_W)) {
        return BTN_UP;
    }
    // Scroll down (aligned with X button)
    if (ui_touch_in_rect(touch_x, touch_y, WIFI_SCROLL_BTN_X, WIFI_LIST_Y + WIFI_LIST_H - WIFI_SCROLL_BTN_W,
                         WIFI_SCROLL_BTN_W, WIFI_SCROLL_BTN_W)) {
        return BTN_DOWN;
    }

    // Enable checkbox (title row)
    if (ui_touch_in_rect(touch_x, touch_y, WIFI_CB_X - 2, WIFI_CB_Y - 2, WIFI_CB_TOUCH_W, WIFI_CB_TOUCH_H)) {
...
static void start_connect()
{
    ...
    if (s_networks[s_selected_idx].secure) {
        ...
        char title[64];
        snprintf(title, sizeof(title), wstr(WIFI_STR_PASSWORD_FOR), s_networks[s_selected_idx].ssid);
...
void wifi_selector_show(int pos_x, int pos_y, wifi_connect_callback_t connect_callback,
                        wifi_scan_callback_t scan_callback, wifi_enable_callback_t enable_callback, bool enabled,
                        void *user_data)
{
    s_x = pos_x;
    s_y = pos_y;
...
void wifi_selector_draw()
{
    ...
    // Background
    gfx_fillRoundRect(s_x, s_y, WIFI_SELECTOR_WIDTH, WIFI_SELECTOR_HEIGHT, UI_RADIUS_NORMAL, UI_COLOR_BG_PANEL);
    gfx_drawRoundRect(s_x, s_y, WIFI_SELECTOR_WIDTH, WIFI_SELECTOR_HEIGHT, UI_RADIUS_NORMAL, UI_COLOR_BORDER);
